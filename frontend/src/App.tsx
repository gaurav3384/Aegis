...
import { Terminal, Shield, Activity, Book, Search, Play, ChevronRight, Info } from 'lucide-react';
...
function App() {
  ...
  const executeCommand = async () => {
    setLoading(true);
    setOutput(`[+] Executing: ${tool} ${params} ${target}\n[!] Authorization Verified...\n----------------------------------------\n`);
    try {
      const res = await axios.post(`${API_BASE}/execute`, { tool, target, params });
      setOutput(prev => prev + (res.data.output || res.data.error || 'Command executed with no output.'));
    } catch (err) {
      setOutput(prev => prev + 'Error: Could not connect to Aegis Engine.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-aegis-900 flex flex-col font-mono selection:bg-aegis-cyan selection:text-black">
      ...
      <nav className="h-16 border-b border-aegis-700 bg-black flex items-center justify-between px-8 shadow-lg shadow-cyan-900/10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black tracking-widest text-white italic">AEGIS // PENTEST_OS</h1>
        </div>
        <div className="flex items-center gap-6 text-xs text-gray-500 uppercase tracking-widest">
          <span className="flex items-center gap-2"><Activity size={14} className="text-green-500" /> Engine: Online</span>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-80 border-r border-aegis-700 bg-aegis-800 p-6 flex flex-col gap-6 overflow-y-auto">
          ...
          <div>
            <label className="text-[10px] uppercase text-aegis-cyan mb-2 block font-bold">Parameters</label>
            <input 
              value={params}
              onChange={(e) => setParams(e.target.value)}
              className="w-full bg-black border border-aegis-700 rounded p-3 text-sm focus:border-aegis-cyan outline-none"
            />
          </div>
          ...
          <button 
            ...
            className="w-full bg-aegis-cyan text-black py-4 rounded font-black uppercase tracking-widest hover:brightness-110 disabled:opacity-50 flex items-center justify-center gap-2 transition-all shadow-lg shadow-cyan-500/20"
          >
          ...
          <div className="mt-4 border-t border-aegis-700 pt-6">
            <h3 className="text-xs font-bold text-white mb-4 flex items-center gap-2 uppercase">
              <Book size={14} className="text-aegis-cyan" /> CheatSheet: {tool}
            </h3>
            <div className="space-y-4">
              {CHEATSHEETS[tool as keyof typeof CHEATSHEETS]?.commands.map((c, i) => (
                <div key={i} className="bg-black p-3 rounded border-l-2 border-aegis-cyan">
                  ...
                  <div className="flex items-start gap-2 text-[10px] text-gray-300 italic">
                    <ChevronRight size={10} className="mt-1 text-aegis-cyan" />
                    <span>Next: {c.next}</span>
                  </div>
                </div>
              ))}
              <a 
                href={CHEATSHEETS[tool as keyof typeof CHEATSHEETS]?.doc} 
                target="_blank" 
                className="text-[10px] text-aegis-cyan hover:underline flex items-center gap-1"
              >
                <Info size={10} /> Full Documentation
              </a>
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 bg-black p-8 flex flex-col gap-4">
          <div className="flex items-center justify-between text-[10px] text-gray-600 uppercase tracking-[0.2em]">
            <span>Console Output // Terminal_ID: {Math.random().toString(36).substring(7)}</span>
            <span>UTF-8 // Authorized Execution</span>
          </div>
      ...
      <footer className="h-8 border-t border-aegis-700 bg-aegis-800 px-8 flex items-center justify-between text-[10px] text-gray-500 uppercase">
        <span>Aegis v1.0.4-Beta // Regression Testing Environment</span>
        <span>Warning: Use only on authorized targets.</span>
      </footer>
    </div>
  );
}
